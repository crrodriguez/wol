 ...
 *
 * $Id: macfile.c,v 1.7 2003/08/09 08:24:23 wol Exp $
 *
...
static unsigned int get_tokens(const char *str, char *mac, char *host, unsigned int *port, char *passwd) {
        if (sscanf(str, " # %*s") == 1) {
                return 0;
        }

        if (sscanf(str, " " MAC_FMT " " HOST_FMT PORT_FMT " " PASS_FMT, mac, host, port, passwd) == 4) {
                return MAC | HOST | PORT | PASS;
        }

        if (sscanf(str, " " MAC_FMT " " HOST_FMT " " PASS_FMT, mac, host, passwd) == 3) {
...
int macfile_parse(FILE *fp, char **mac_str, char **host_str, unsigned int *port, char **passwd_str) {
        char *willy = NULL;
        ssize_t ret;
        size_t whale = 0;
